...
static float viewDist = 3; // Distance from the camera to the centre of the scene. Doubled for part D.
...
void init(void) {
    ...
    //shaderProgram = InitShader("res/init_shaders/vStart.glsl", "res/init_shaders/fStart.glsl");

    // Load Shaders for Parts A-F
    //shaderProgram = InitShader("res/shaders/vStart.glsl", "res/shaders/fStart.glsl");

    // Load shaders for the rest
    shaderProgram = InitShader("res/new_shaders/vStart.glsl", "res/new_shaders/fStart.glsl");
    ...
    //Part I
    //  Need to add sphere for second light
    addObject(55);
    sceneObjs[2].loc = vec4(4.0, 0.1, 0.1, 0.1);
...
void drawMesh(SceneObject sceneObj) {
    ...
    mat4 rot_xyz = RotateX(sceneObj.angles[0]) * RotateY(sceneObj.angles[1]) * RotateZ(sceneObj.angles[2]);
    mat4 model = Translate(sceneObj.loc) * Scale(sceneObj.scale) * rot_xyz;
...
void display(void) {
    ...
    //Part A
    // You can move it forwards and backwards using the scroll wheel, via the viewDist variable, 
    // The variables camRotUpAndOverDeg and camRotSidewaysDeg are already set appropriately when 
    // moving the mouse with the left button down, or the middle button down (or shift + left button).
    // modify display function
    mat4 rotX = RotateX(camRotUpAndOverDeg);
    mat4 rotY = RotateY(camRotSidewaysDeg);
    view = Translate(0.0, 0.0, -viewDist) * rotX * rotY;
    ...
    vec4 lightPosition2 = rotX * rotY * lightObj2.loc;
    ...
    glUniform4fv(glGetUniformLocation(shaderProgram, "LightColor"),
                 1, lightObj1.rgb);
    CheckError();
    glUniform4fv(glGetUniformLocation(shaderProgram, "LightColor2"),
                 1, lightObj2.rgb);
    CheckError();
    glUniform4fv(glGetUniformLocation(shaderProgram, "LightColor3"),
                 1, lightObj3.rgb);
    CheckError();
...
static void adjustAmbience_Diffuse(vec2 ad)
{
...
static void adjustSpec_Shine(vec2 spsh)
{
    sceneObjs[toolObj].specular += spsh[0];
    sceneObjs[toolObj].specular += spsh[1];
}
...
/*    if(id == 101)
    {
        toolObj = 3;
        setToolCallbacks(camRotZ,adjustBrightnessY, mat2(1.0, 0, 0, 1.0));
    }*/
    else 
    {
        printf("Error in spotlightMenu\n");
        exit(1);
    }
}
...
static void materialMenu(int id) {
    ...
    if (id == 10) {
        ...
        //Part C 
        // Implement a menu item for adjusting the amounts of ambient
        // ,diffuse, and specular light, as well as the amount of shine. 
        // Modify the materialMenu and makeMenuM functions so that 
        // they change the appropriate members of the SceneObject structure
    else if (id == 20)
    {
        toolObj = currObject;
        setToolCallbacks(adjustAmbience_Diffuse, mat2(1, 0, 0, 1), adjustSpec_Shine, mat2(1, 0, 0, 20));
    }
    else {
...
static void deleteObject(int current_id)
{
    if(nObjects == 4) // no object to delete
    {
        return;
    }
    nObjects--;
...
static void duplicateObject(int current_id)
{
    if(nObjects == 4) //ground 0, light 1, light 2, light 3
    {
        return;
    }
    sceneObjs[nObjects] = sceneObjs[current_id];
    toolObj = current_id = nObjects++;

    setToolCallbacks(adjustLocXZ, camRotZ(), adjustScaleY, mat2(0.5, 0, 0, 10));
...
static void makeMenu() {
    ...
    //glutAddMenuEntry("Change Direction", 101);
...
void reshape(int width, int height) {
    ...
    // The default viewing volume is a box centered at
    // the origin with sides of length 2 (±1 in each direction)
    //Part D
    ...
    if(width > height)
    {
        ...
        -nearDist * (float)height / (float)width, nearDist * (float)height / (float)width, nearDist, 100.0);
    }
    else
    {
